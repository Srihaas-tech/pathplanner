...
class Path2PointTowardsZonesTree extends StatefulWidget {
  ...
  const Path2PointTowardsZonesTree({
    super.key,
    required this.path,
  ...
  @override
  State<Path2PointTowardsZonesTree> createState() =>
      _Path2PointTowardsZonesTreeState();
}

class _Path2PointTowardsZonesTreeState
    extends State<Path2PointTowardsZonesTree> {
  ...
  Widget build(BuildContext context) {
    ...
    return TreeCardNode(
      ...
      onExpansionChanged: (expanded) {
        ...
        widget.path.pointTowardsZonesExpanded = expanded;
    ...
    return TreeCardNode(
      ...
      children: [
            ...
            children: [
              Expanded(
                child: NumberTextField(
                  key: ValueKey('path2PointZone${zoneIdx}FieldX'),
                  initialValue: zone.fieldPosition.x,
                  label: 'Field Position X (M)',
                  onSubmitted: (value) {
                    if (value != null) {
                      _addZoneChange(
                        zoneIdx,
                        (updated) => updated.fieldPosition = Translation2d(
                          value,
                          updated.fieldPosition.y,
                        ),
                      );
                    }
                  },
              ...
              Expanded(
                child: NumberTextField(
                  key: ValueKey('path2PointZone${zoneIdx}FieldY'),
                  initialValue: zone.fieldPosition.y,
        ...
        Padding(
          padding: const EdgeInsets.symmetric(horizontal: 6),
          child: NumberTextField(
            key: ValueKey('path2PointZone${zoneIdx}RotationOffset'),
            initialValue: zone.rotationOffset.degrees,
            ...
            onSubmitted: (value) {
              if (value != null) {
                _addZoneChange(
                  zoneIdx,
                  (updated) => updated.rotationOffset = Rotation2d.fromDegrees(
        ...
        const SizedBox(height: 12),
        Tooltip(
          message: 'Use a normal PID controller for rotation while inside '
              'this zone instead of the profiled rotation controller.',
          child: CheckboxListTile(
            ...
            value: zone.unprofiled,
            onChanged: (value) {
              _addZoneChange(
                zoneIdx,
                (updated) => updated.unprofiled = value ?? false,
              );
            },
          ),
        ),
  ...
  void _addZone() {
    ...
    final newZone = PointTowardsZone();
    newZone.minWaypointRelativePos =
        newZone.minWaypointRelativePos.clamp(0, _maxPosition);
    newZone.maxWaypointRelativePos =
        newZone.maxWaypointRelativePos.clamp(0, _maxPosition);
    final after = _cloneZones(zones)..add(newZone);
  ...
  void _addZonesListChange(
    List<PointTowardsZone> before,
    List<PointTowardsZone> after, {
    bool clearSelection = false,
  }) {
    widget.undoStack.add(Change<List<PointTowardsZone>>(
      before,
  ...
  void _replaceZones(
    List<PointTowardsZone> replacement, {
    required bool clearSelection,
  }) {
    _apply(() {
      widget.path.pointTowardsZones = _cloneZones(replacement);
  ...
  void _addZoneChange(
    int zoneIdx,
    ValueChanged<PointTowardsZone> mutate, {
    bool simulate = true,
  }) {
    final before = zones[zoneIdx].clone();
    final after = before.clone();
    mutate(after);
    widget.undoStack.add(Change<PointTowardsZone>(
      before,
      () => _apply(
  ...
  static List<PointTowardsZone> _cloneZones(Iterable<PointTowardsZone> zones) =>
